...
export function Conversations(): React.JSX.Element {
  ...
  const generateIndividualChats = (conversations: ConversationResponse[]): IndividualChat[] => {
    const individualChats: IndividualChat[] = [];
    
    conversations.forEach(conversation => {
      if (conversation.type === "TRIP" && conversation.participants.length > 1) {
        const driverId = conversation.participants[0];
        
        conversation.participants.slice(1).forEach((passengerId, index) => {
          individualChats.push({
            id: `${conversation.id}-${passengerId}`, 
            conversationId: conversation.id, 
            participantId: passengerId,
            participantName: `Pasajero #${index + 1} (${passengerId})`,
            tripId: conversation.travelId,
        ...
        individualChats.push({
          id: conversation.id,
          conversationId: conversation.id,
          participantId: conversation.participants[0] || 0,
          participantName: `Usuario ${conversation.participants[0]}`,
          tripId: conversation.travelId,
          type: conversation.type
        });
      }
  ...
  const loadConversations = async (): Promise<void> => {
    try {
      ...
      const chats = generateIndividualChats(data);
  ...
  const handleSelectChat = async (chat: IndividualChat): Promise<void> => {
    setSelectedChat(chat);
    try {
      setError("");

      const messagesData = await conversationApi.getMessages(chat.conversationId);
  ...
  const handleSendMessage = async (): Promise<void> => {
    ...
    try {
      ...
      if (sentMessage) {
        ...
        const updatedMessages = await conversationApi.getMessages(selectedChat.conversationId);
  ...
  return (
    <div className="flex h-screen bg-white p-6 gap-6">
      {/* RECTÁNGULO 1: Lista de chats individuales */}
